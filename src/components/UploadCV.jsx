...
export default function UploadCV({ onCVParsed }) {
  ...
  useEffect(() => {
    ...
    if (savedFile) setFile(JSON.parse(savedFile));
    if (savedCV) onCVParsed(JSON.parse(savedCV));
  }, []);
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      console.error(err);
      alert("Error parsing CV. Make sure backend is running and reachable.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <section className="relative w-full h-[600px] max-w-7xl mx-auto px-4 py-1">
      ...
      >
        <div>
          ...
          <label
            htmlFor="cv-upload"
            className="flex flex-col items-center justify-center gap-4 p-10 border-2 border-dashed rounded-2xl cursor-pointer transition-all duration-300 border-gray-300 hover:border-[#faa302] hover:bg-gray-500"
          >
        ...
        <div className="relative group w-full">
          <button
            disabled
            className="mt-6 w-full bg-[#faa302] text-white py-3 rounded-xl font-semibold transition-all duration-300 shadow-lg cursor-not-allowed opacity-70"
          >
            Check ATS Score
          </button>
